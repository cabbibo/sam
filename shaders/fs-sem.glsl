...
void main(){
  ...
  vec4 aC1 = texture2D( t_audio , vec2( abs(dot(vNorm , normalize( vCamDir ))), 0. ) );
 ...
 // float refl = vMPos - lightPos
  ...
  vec4 aC = texture2D( t_audio , vec2( abs(sin(dot(  normalize( vCamDir ) , vMNorm ))) , 0. )  );
 ...
 // gl_FragColor = vec4( vec3( .5 , .4 , .2 ) + vec3( 1. , 1. , .6 ) * aC.xyz * aC1.xyz , 1. ); //aC ; //* vec4(  1000. - vMPos.y , 100. / vMPos.y , .3, 1. );
  ...
  gl_FragColor = sem * aC1;// + vec4( 0.5 * normalize(vReflection ) + 0.7 , 1. ) *   sem; //vec4( vSEM.x , 0. , vSEM.y , 1. );
